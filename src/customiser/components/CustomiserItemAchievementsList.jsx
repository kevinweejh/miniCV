...
const CustomiserItemAchievementsList = ({ achievementsList, handleAchievementsListAdd }) => {
    ...
    const handleAddClick = () => {
        if (newAchievement.trim()) { // Check if the input is not just empty spaces
    ...
    return(
        <>
            ...
            <input id="newAchievement" type="text" value={newAchievement} onChange={e => setNewAchievement(e.target.value)} placeholder="Enter achievement"></input>
            <button onClick={handleAddClick} type="button" className="border rounded-md w-fit mt-4 ml-auto px-2 border-gray-400 hover:bg-gray-400">Add achievement</button>
        </>
    )
}
